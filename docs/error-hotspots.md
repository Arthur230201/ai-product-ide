...
14. **app/actions/node-operations.ts:26**
   - 匹配: `rate_limit`
   - 上下文: ` { type: 'success'; code: string; requestId?: string; stages?: unknown; viewportUsed?: 'mobile' | 'desktop' }
  | { type: 'skeleton'; code: string; re...`

15. **app/actions/node-operations.ts:191**
   - 匹配: `RATE_LIMIT`
   - 上下文: `ge,
...
16. **app/actions/node-operations.ts:894**
   - 匹配: `RATE_LIMIT`
...
17. **app/actions/node-operations.ts:896**
   - 匹配: `rate_limit`
   - 上下文: `,
          elapsedMs: apiDuration,
            });
...
18. **app/actions/node-operations.ts:1156**
   - 匹配: `RATE_LIMIT`
   - 上下文: `
          model: textModel,
          errorType: result.type,
...
19. **app/actions/node-operations.ts:1158**
   - 匹配: `rate_limit`
   - 上下文: `ult.type,
          elapsedMs: apiDuration,
        });
...
20. **app/actions/node-operations.ts:1514**
   - 匹配: `RATE_LIMIT`
   - 上下文: `rPrompt}`,
        temperature: 0.5,
...
21. **app/actions/node-operations.ts:26**
   - 匹配: `cooldown`
   - 上下文: `cess'; code: string; requestId?: string; stages?: unknown; viewportUsed?: 'mobile' | 'desktop' }
  | { type: 'skeleton'; code: string; requestId?: str...`

22. **app/actions/node-operations.ts:191**
   - 匹配: `cooldown`
   - 上下文: `    };
  }
}
...
23. **app/actions/node-operations.ts:897**
   - 匹配: `cooldown`
   - 上下文: `,
            });
...
24. **app/actions/node-operations.ts:897**
   - 匹配: `cooldown`
   - 上下文: `  
        // Map AIResult error types to return format
        if (result.type === 'RATE_LIMIT') {
...
25. **app/actions/node-operations.ts:1159**
   - 匹配: `cooldown`
   - 上下文: `Duration,
        });
...
26. **app/actions/node-operations.ts:1159**
   - 匹配: `cooldown`
   - 上下文: `      
        // Map AIResult error types to return format
        if (result.type === 'RATE_LIMIT') {
...
27. **app/actions/node-operations.ts:1515**
   - 匹配: `cooldown`
...
28. **app/actions/ui-pipeline-response.test.ts:61**
   - 匹配: `RATE_LIMIT`
   - 上下文: `rue);
...
29. **app/actions/ui-pipeline-response.test.ts:70**
   - 匹配: `RATE_LIMIT`
   - 上下文: `s',
    cooldownSeconds: 10,
    retryable: true,
...
30. **app/actions/ui-pipeline-response.test.ts:112**
   - 匹配: `Rate limit`
...
31. **app/actions/ui-pipeline-response.test.ts:118**
   - 匹配: `RATE_LIMIT`
...
32. **app/actions/ui-pipeline-response.test.ts:120**
   - 匹配: `Rate limit`
   - 上下文: `ndefined, error];
  const result = normalizeUIPipelineResponse(tuple);
...
33. **app/actions/ui-pipeline-response.test.ts:62**
...
34. **app/actions/ui-pipeline-response.test.ts:63**
   - 匹配: `cooldown`
...
35. **app/actions/ui-pipeline-response.test.ts:113**
   - 匹配: `cooldown`
...
36. **app/actions/ui-pipeline-response.test.ts:121**
   - 匹配: `cooldown`
...
37. **app/actions/ui-pipeline-response.ts:28**
   - 匹配: `RATE_LIMIT`
   - 上下文: `ineResponse =
  | {
      ok: true;
...
38. **app/actions/ui-pipeline-response.ts:42**
   - 匹配: `RATE_LIMIT`
...
39. **app/actions/ui-pipeline-response.ts:47**
   - 匹配: `RATE_LIMIT`
   - 上下文: `or types
...
40. **app/actions/ui-pipeline-response.ts:48**
   - 匹配: `RATE_LIMIT`
...
41. **app/actions/ui-pipeline-response.ts:92**
   - 匹配: `RATE_LIMIT`
...
42. **app/actions/ui-pipeline-response.ts:113**
   - 匹配: `RATE_LIMIT`
...
45. **app/actions/ui-pipeline-response.ts:30**
   - 匹配: `cooldown`
   - 上下文: `   html: string;
      meta: HTMLMeta;
      warnings?: string[];
...
46. **app/actions/ui-pipeline-response.ts:37**
   - 匹配: `cooldown`
...
47. **app/actions/ui-pipeline-response.ts:73**
   - 匹配: `cooldown`
   - 上下文: ` break;
      default:
...
49. **app/actions/ui-pipeline-response.ts:109**
   - 匹配: `cooldown`
   - 上下文: `aw;
...
50. **app/actions/ui-pipeline-response.ts:110**
   - 匹配: `cooldown`
...
52. **app/actions/ui-pipeline-response.ts:124**
   - 匹配: `cooldown`
...
53. **components/canvas/CommandBar.tsx:540**
   - 匹配: `429`
   - 上下文: `out') || error.message.includes('TIMEOUT')) {
          errorMessage = '请求超时，请检查网络连接';
        } else if (error.message.includes('quota') || error.mes...`

54. **components/canvas/CommandBar.tsx:769**
   - 匹配: `429`
   - 上下文: `EOUT')) {
          errorMessage = '⏱️ 请求超时\n\n请检查网络连接，或稍后重试\n\n提示：可以尝试简化输入内容或减少附件大小';
        } else if (error.message.includes('quota') || error.mes...`

55. **components/canvas/CommandBar.tsx:1556**
   - 匹配: `rate_limit`
...
56. **components/canvas/CommandBar.tsx:2550**
   - 匹配: `rate_limit`
   - 上下文: `         
...
57. **components/canvas/CommandBar.tsx:1558**
   - 匹配: `cooldown`
   - 上下文: `sult) {
              const response = uiResult;
...
58. **components/canvas/CommandBar.tsx:2552**
   - 匹配: `cooldown`
...
59. **components/canvas/CommandBar.tsx:2553**
   - 匹配: `cooldown`
   - 上下文: ` (response.type) {
            case 'rate_limit': {
              toast.error('请求过多', {
...
60. **components/canvas/EditMode.tsx:120**
   - 匹配: `Retry after`
...
137. **lib/ai/llm.ts:481**
   - 匹配: `cooldown`
   - 上下文: `el}:${actionName}:${promptHash || messagesHash}:${attachmentsHash}:${mode}`;
  const keyHash = stableHash(keyInput);
  const taskKey: GatewayTaskKey =...`

138. **lib/ai/llm.ts:482**
   - 匹配: `cooldown`
   - 上下文: `sHash}:${attachmentsHash}:${mode}`;
  const keyHash = stableHash(keyInput);
  const taskKey: GatewayTaskKey = `${model}:${actionName}:${keyHash}`;
...
190. **lib/ui-generation-helpers.ts:94**
   - 匹配: `429`
   - 上下文: `object
...
191. **lib/ui-generation-helpers.ts:102**
   - 匹配: `429`
   - 上下文: `unknown): boolean {
  if (!error || typeof error !== 'object') return false;
...
192. **lib/ui-generation-helpers.ts:106**
   - 匹配: `429`
   - 上下文: `es('429') ||
    errorMessage.includes('Too Many Requests') ||
    errorMessage.includes('rate limit') ||
...
193. **lib/ui-generation-helpers.ts:107**
   - 匹配: `429`
   - 上下文: `quests') ||
    errorMessage.includes('rate limit') ||
    errorMessage.includes('Rate limit') ||
...
194. **lib/ui-generation-helpers.ts:6**
   - 匹配: `Rate limit`
   - 上下文: `/**
...
195. **lib/ui-generation-helpers.ts:94**
   - 匹配: `rate limit`
...
196. **lib/ui-generation-helpers.ts:96**
   - 匹配: `RateLimit`
...
197. **lib/ui-generation-helpers.ts:104**
   - 匹配: `rate limit`
...
198. **lib/ui-generation-helpers.ts:105**
   - 匹配: `Rate limit`
...
199. **lib/ui-generation-helpers.ts:198**
   - 匹配: `RATE_LIMIT`
...
200. **lib/ui-generation-helpers.ts:204**
   - 匹配: `rate limit`
   - 上下文: `
...
201. **lib/ui-generation-helpers.ts:222**
   - 匹配: `Rate limit`
   - 上下文: `omise<T>,
  requestId: string,
  model: string
...
202. **lib/ui-generation-helpers.ts:225**
   - 匹配: `Rate limit`
   - 上下文: `
...
203. **lib/ui-generation-helpers.ts:226**
   - 匹配: `RATE_LIMIT`
   - 上下文: `
      // Rate limit fast-fail timeout
      new Promise<never>((_, reject) => {
...
204. **lib/ui-generation-helpers.ts:234**
   - 匹配: `rate limit`
   - 上下文: `imeout'));
...
205. **lib/ui-generation-helpers.ts:235**
   - 匹配: `RateLimit`
...
206. **lib/ui-generation-helpers.ts:237**
   - 匹配: `Rate Limit`
   - 上下文: `rror) {
    const elapsed = Date.now() - startTime;
...
207. **lib/ui-generation-helpers.ts:244**
   - 匹配: `rateLimit`
   - 上下文: `wnSeconds(error);
...
208. **lib/ui-generation-helpers.ts:244**
   - 匹配: `Rate limit`
...
209. **lib/ui-generation-helpers.ts:245**
   - 匹配: `rate_limit`
   - 上下文: `    requestId,
        model,
        elapsedMs: elapsed,
...
210. **lib/ui-generation-helpers.ts:248**
   - 匹配: `rateLimit`
...
211. **lib/ui-generation-helpers.ts:248**
   - 匹配: `rate_limit`
   - 上下文: `ooldown,
      });
...
212. **lib/ui-generation-helpers.ts:249**
   - 匹配: `rateLimit`
   - 上下文: `      
      const rateLimitError = new Error('Rate limit exceeded') as Error & {
        type: 'rate_limit';
...
213. **lib/ui-generation-helpers.ts:250**
   - 匹配: `rateLimit`
...
214. **lib/ui-generation-helpers.ts:254**
   - 匹配: `RATE_LIMIT`
   - 上下文: `;
...
215. **lib/ui-generation-helpers.ts:103**
   - 匹配: `Too Many Requests`
...
216. **lib/ui-generation-helpers.ts:65**
   - 匹配: `cooldown`
   - 上下文: `ng,
  timing: StageTiming
): void {
...
217. **lib/ui-generation-helpers.ts:68**
   - 匹配: `Cooldown`
...
218. **lib/ui-generation-helpers.ts:90**
   - 匹配: `cooldown`
   - 上下文: `  }
    }
...
219. **lib/ui-generation-helpers.ts:236**
   - 匹配: `cooldown`
   - 上下文: `IL_MS);
      }),
    ]);
...
220. **lib/ui-generation-helpers.ts:236**
   - 匹配: `Cooldown`
   - 上下文: `    ]);
...
221. **lib/ui-generation-helpers.ts:241**
   - 匹配: `cooldown`
   - 上下文: `imitError(error)) {
...
222. **lib/ui-generation-helpers.ts:241**
   - 匹配: `cooldown`
   - 上下文: ` {
      const cooldown = extractCooldownSeconds(error);
      logError('🚫 [Rate Limit] Fast-fail triggered', {
...
223. **lib/ui-generation-helpers.ts:246**
   - 匹配: `cooldown`
   - 上下文: `  model,
        elapsedMs: elapsed,
        cooldownSeconds: cooldown,
...
224. **lib/ui-generation-helpers.ts:249**
   - 匹配: `cooldown`
...
225. **lib/ui-generation-helpers.ts:249**
   - 匹配: `cooldown`
...
226. **lib/ui-generation-helpers.ts:65**
   - 匹配: `Retry-After`
   - 上下文: `ing
): void {
  log(`⏱️ [${timing.stage}] ${timing.name}`, {
...
227. **lib/ui-generation-helpers.ts:70**
   - 匹配: `Retry-After`
...
228. **lib/ui-generation-helpers.ts:74**
   - 匹配: `retryAfter = (headers as { get: (name: string) => string | null }).get('retry-after`
   - 上下文: `rs = 'headers' in error.response ? error.response.headers : null;
      if (headers != null && typeof headers === 'object' && typeof (headers as { get...`

229. **lib/ui-generation-helpers.ts:75**
   - 匹配: `retryAfter`
   - 上下文: `of headers === 'object' && typeof (headers as { get?: unknown }).get === 'function') {
        const retryAfter = (headers as { get: (name: string) =>...`

230. **lib/ui-generation-helpers.ts:76**
   - 匹配: `retryAfter`
   - 上下文: `et?: unknown }).get === 'function') {
        const retryAfter = (headers as { get: (name: string) => string | null }).get('retry-after');
        if ...`

231. **lib/ui-generation-helpers.ts:84**
   - 匹配: `retryAfter`
   - 上下文: `retryAfter) {
          const seconds = parseInt(retryAfter, 10);
          if (!isNaN(seconds) && seconds > 0) {
...
232. **lib/ui-generation-helpers.ts:85**
...
233. **lib/ui-generation-helpers.ts:86**
   - 匹配: `retryAfter`
   - 上下文: `       return seconds;
          }
        }
...
234. **lib/user-facing-messages.ts:20**
   - 匹配: `RATE_LIMIT`
   - 上下文: `
...
235. **lib/user-facing-messages.ts:17**
   - 匹配: `cooldown`
...
236. **lib/user-facing-messages.ts:21**
   - 匹配: `cooldown`
   - 上下文: `发环境或日志。
 */
export function getUserFacingErrorDescription(
...
237. **lib/user-facing-messages.ts:21**
   - 匹配: `cooldown`
...
238. **lib/user-facing-messages.ts:22**
   - 匹配: `cooldown`
...
239. **utils/prdGenerator.ts:1396**
   - 匹配: `Retry after`
...
5. **app/actions/node-operations.ts:323**
   - 匹配: `.substring(`
   - 有类型检查: 否
   - 上下文: `'').trim();
      })
...
6. **app/actions/node-operations.ts:454**
   - 匹配: `.substring(`
   - 有类型检查: 否
...
7. **app/actions/node-operations.ts:670**
   - 匹配: `.substring(`
   - 有类型检查: 否
   - 上下文: `数据格式无效或数据过短');
      }
...
8. **app/actions/node-operations.ts:844**
   - 匹配: `.substring(`
   - 有类型检查: 否
   - 上下文: `ode now.`;
...
9. **app/actions/node-operations.ts:926**
   - 匹配: `.substring(`
   - 有类型检查: 否
...
10. **app/actions/node-operations.ts:936**
   - 匹配: `.substring(`
   - 有类型检查: 否
   - 上下文: `pt|javascript)?\n?/gm, '')
        .replace(/\n?```$/gm, '')
...
11. **app/actions/node-operations.ts:1194**
   - 匹配: `.substring(`
   - 有类型检查: 否
...
12. **app/actions/node-operations.ts:1232**
   - 匹配: `.substring(`
   - 有类型检查: 否
...
13. **app/actions/node-operations.ts:1311**
   - 匹配: `.substring(`
   - 有类型检查: 否
   - 上下文: `  ];
...
14. **app/actions/node-operations.ts:1536**
   - 匹配: `.substring(`
   - 有类型检查: 是
   - 上下文: `
      ];
...
15. **components/canvas/CommandBar.tsx:2601**
   - 匹配: `uiCode.substring`
   - 有类型检查: 否
   - 上下文: `stId,
...
16. **components/canvas/CommandBar.tsx:2612**
   - 匹配: `uiCode.substring`
   - 有类型检查: 否
...
17. **components/canvas/CommandBar.tsx:2634**
   - 匹配: `uiCode.substring`
   - 有类型检查: 否
...
18. **components/canvas/CommandBar.tsx:149**
   - 匹配: `.substring(`
   - 有类型检查: 是
...
19. **components/canvas/CommandBar.tsx:164**
   - 匹配: `.substring(`
   - 有类型检查: 否
...
20. **components/canvas/CommandBar.tsx:236**
   - 匹配: `.substring(`
   - 有类型检查: 否
...
21. **components/canvas/CommandBar.tsx:248**
   - 匹配: `.substring(`
   - 有类型检查: 是
...
22. **components/canvas/CommandBar.tsx:297**
   - 匹配: `.substring(`
   - 有类型检查: 是
...
23. **components/canvas/CommandBar.tsx:325**
   - 匹配: `.substring(`
   - 有类型检查: 否
...
24. **components/canvas/CommandBar.tsx:339**
   - 匹配: `.substring(`
   - 有类型检查: 否
...
25. **components/canvas/CommandBar.tsx:1708**
   - 匹配: `.substring(`
   - 有类型检查: 否
...
26. **components/canvas/CommandBar.tsx:1732**
   - 匹配: `.substring(`
   - 有类型检查: 否
...
27. **components/canvas/CommandBar.tsx:1747**
   - 匹配: `.substring(`
   - 有类型检查: 否
...
28. **components/canvas/CommandBar.tsx:1779**
   - 匹配: `.substring(`
   - 有类型检查: 否
   - 上下文: `证:', {
                nodeId: targetNodeId,
...
29. **components/canvas/CommandBar.tsx:1985**
   - 匹配: `.substring(`
   - 有类型检查: 否
   - 上下文: `actClone,
...
30. **components/canvas/CommandBar.tsx:2169**
   - 匹配: `.substring(`
   - 有类型检查: 否
...
31. **components/canvas/CommandBar.tsx:2601**
   - 匹配: `.substring(`
   - 有类型检查: 否
...
32. **components/canvas/CommandBar.tsx:2612**
   - 匹配: `.substring(`
   - 有类型检查: 否
...
33. **components/canvas/CommandBar.tsx:2632**
   - 匹配: `.substring(`
   - 有类型检查: 否
   - 上下文: `?.code || '';
...
34. **components/canvas/CommandBar.tsx:2634**
   - 匹配: `.substring(`
   - 有类型检查: 否
...
35. **components/canvas/CommandBar.tsx:2634**
   - 匹配: `.substring(`
   - 有类型检查: 否
...
36. **components/canvas/CommandBar.tsx:2823**
   - 匹配: `.substring(`
   - 有类型检查: 是
...
37. **components/canvas/CommandBar.tsx:2871**
   - 匹配: `.substring(`
   - 有类型检查: 否
...
38. **components/canvas/HtmlFirstPreview.tsx:585**
   - 匹配: `.substring(`
   - 有类型检查: 否
...
39. **components/canvas/HtmlFirstPreview.tsx:611**
   - 匹配: `.substring(`
   - 有类型检查: 否
   - 上下文: `L rendering using HtmlSandbox
          // ⚠️ 重要：使用 htmlSource prop 而不是 ref，确保响应式更新
          <div className="w-full h-full relative">
            <Ht...`

40. **components/canvas/ProjectBlueprint.tsx:458**
   - 匹配: `.substring(`
   - 有类型检查: 否
...
41. **components/canvas/ProjectBlueprint.tsx:463**
   - 匹配: `.substring(`
   - 有类型检查: 否
   - 上下文: `(0, 30);
...
42. **components/canvas/ProjectBlueprint.tsx:465**
   - 匹配: `.substring(`
   - 有类型检查: 否
   - 上下文: `> {
...
43. **components/canvas/WordUploader.tsx:105**
   - 匹配: `.substring(`
   - 有类型检查: 否
...
45. **lib/ai/gateway-queue.ts:167**
   - 匹配: `.substring(`
   - 有类型检查: 否
...
46. **lib/ai/gateway-queue.ts:195**
   - 匹配: `.substring(`
   - 有类型检查: 否
...
53. **lib/preview-ui-prd-bundle.generated.ts:399**
   - 匹配: `.substring(`
   - 有类型检查: 否
...
60. **lib/ui-generation-helpers.ts:158**
   - 匹配: `.substring(`
   - 有类型检查: 否
...
61. **lib/ui-generation-helpers.ts:163**
   - 匹配: `.substring(`
   - 有类型检查: 否
...
73. **utils/ui-summary-extractor.ts:110**
   - 匹配: `.substring(`
   - 有类型检查: 否
   - 上下文: `ectorAll('li');
...
74. **utils/ui-summary-extractor.ts:124**
   - 匹配: `.substring(`
   - 有类型检查: 否
   - 上下文: `tle"]');
...
75. **utils/ui-summary-extractor.ts:125**
   - 匹配: `.substring(`
   - 有类型检查: 否
   - 上下文: `tent?.trim() || '';
      const content = card.textContent?.trim() || '';
...
76. **utils/ui-summary-extractor.ts:197**
   - 匹配: `.substring(`
   - 有类型检查: 否
   - 上下文: `<\/li>/gi);
    let liCount = 0;
